  ...
  return (
    <div className="grid grid-cols-3 md:flex items-center  gap-3 overflow-x-auto py-6 no-scrollbar">
      {KategoriIcon.map((category) => {
        ...
        const isActive = activeCategory === category.name; // kategori yang aktif
        return (
          <button
            key={category.name}
            onClick={() => handleFilter(category.name)}
            className={`mx-1 px-4 py-2 rounded-full whitespace-nowrap text-xs md:text-sm  font-medium transition-all flex gap-2 items-center justify-center hover:scale-105 duration-100 cursor-pointer
            ${
              isActive
