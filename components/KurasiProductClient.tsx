...
import { VerifyProduct } from "@/lib/action";
import { ProductAdminType } from "@/types/product";
import { CheckCircle, XCircle, Eye, ShoppingBag, Clock } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { toast } from "sonner";

const KurasiProductClient = ({
  pendingProducts,
}: {
  pendingProducts: ProductAdminType[];
}) => {
  const handleVerify = async (id: string, data: string) => {
    if (!id || !data) {
      return;
    }

    toast.promise(VerifyProduct(id, data), {
            ...
            >
              ...
              <div className="p-8 flex-1 space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <Link
                    href={`/admin/kurasi-products/${product.slug}`}
                    className="p-3 bg-gray-50 dark:bg-gray-800 rounded-2xl text-gray-400 hover:text-orange-600 transition-colors"
                  >
                    <Eye size={20} />
                  </Link>
                </div>
                ...
                <div className="flex items-center gap-3 py-4 border-y border-gray-50 dark:border-gray-800">
                  ...
                  <div className="text-xs">
                    ...
                    <p className="text-gray-900 dark:text-white font-bold">
                      {product.toko?.user?.name as string}
                    </p>
                  </div>
                </div>
              ...
              <div className="p-6 pt-0 grid grid-cols-2 gap-4">
                <button
                  ...
                  className="flex items-center justify-center gap-2 py-4 bg-gray-50 dark:bg-gray-800 text-red-500 rounded-2xl font-black text-sm hover:bg-red-50 transition-all active:scale-95"
                >
                  <XCircle size={18} /> Tolak
                </button>
                <button
                  onClick={() => handleVerify(product.id, "Approved")}
                  className="flex items-center justify-center gap-2 py-4 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-2xl font-black text-sm hover:bg-orange-600 dark:hover:bg-orange-600 dark:hover:text-white transition-all shadow-lg active:scale-95"
                >
...
export default KurasiProductClient;
